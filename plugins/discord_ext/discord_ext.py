...
class Bot(commands.Bot):
    ...
    def _remove_listener(self, fallback: Callable[[discord.Message], Any]) -> None:
        
        for listeners in self.__messages_listeners.values():

            try:
                listeners.remove(fallback)
...
class DiscordExt(plugins.Plugin[Config, None]):
    ...
    __loaded_cogs: Dict[plugins.Plugin, List[str]]
    ...
    def on_load(self):
        ...
        self.load_cog("online", self)


    def load_cog(self, name: str, plugin: plugins.Plugin):
        """
        Loads the given cog to the bot and binds the plugin
        """

        name = f"plugins.{plugin.name}.{name}"
        ...
        try:
            ...
            setattr(self.__bot.extensions[name], "plugin", plugin)

            self.__loaded_cogs.setdefault(plugin, []).append(name)

        except Exception as e:
            raise e
        

    def on_plugin_unloaded(self, plugin):
        
        for name in self.__loaded_cogs.get(plugin, []):
    ...
    def subscribe_to(self, *channels: int, fallback: Callable[[discord.Message], Any]):
        """
        Calls the fallback every time that a new messages arrives in the given channel(s).

        **IMPORTANT**: You must call unsubscribe_from() yourself when you unload the plugin that contains the fallback!
        """
        
        self.__bot._add_listener(*channels, fallback=fallback)

    
    def unsubscribe_from(self, fallback: Callable[[discord.Message], Any]):
        """
        Removes the given fallback subscribed to new messages
        """

        self.__bot._remove_listener(fallback)
