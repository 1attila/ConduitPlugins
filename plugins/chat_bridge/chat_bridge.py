...
class ChatBridge(plugins.Plugin[Config, None]):
    """
    ...
    def about_to_stop(self):
        ...
        if self.__discord_enabled is True:
            self.__discord_ext.unsubscribe_from(self._on_discord_message)
        

    def get_discord(self):
        """
        Asserts that discord_ext plugin is always working
        """

        #TODO: this works if discord_ext is reloaded, but only mc --> dc
        # To be fixed correctly, conduit should handle plugin manipulation of plugin-dependencies differently 
    ...
    def on_player_chat(self, ctx: Context):
        ...
        for filter in self.config.message_filters:

            if re.match(filter, ctx.message):
                return
    ...
    def _server_broadcast(self, message: Message):
        ...
        if self.manager.get_plugin_named("discord_ext") != self.__discord_ext:
            self.get_discord()
    ...
    def _on_discord_message(self, message: "discord.Message"):
        ...
        msg = text.gray(f"[Discord] <{message.author}> ")
        msg.hover(message.created_at.strftime("%H:%M:%S"))
        msg += text.gray(message.content)
