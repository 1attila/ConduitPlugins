...
class FreeCam(plugins.Plugin[Config, Persistent]):
    ...
    @freecam.command(name="enable-chat")
    def enable_chat(self, ctx: Context):
        """
        No more chat messages will appear if you join/exit freecam
        """

        if ctx.player.name not in self.persistent.disabled_players:
            self.persistent.disabled_players.append(ctx.player.name)

        else:
            ctx.error("You have already enabled freecam-chat!")

    
    @freecam.command(name="disable-chat")
    def disable_chat(self, ctx: Context):
        """
        No more chat messages will appear if you join/exit freecam
        """

        if ctx.player.name in self.persistent.disabled_players:
            self.persistent.disabled_players.pop(ctx.player.name)

        else:
            ctx.error("You have already disabled freecam-chat!")


    @plugins.command(aliases=["spec"])
    def spectate(self, ctx: Context, player: str):
        """
        ...
        self.server.execute(f"/execute in {player.dimension.value} run tp {ctx.player} {player.pos}")
    ...
    def join_freecam(self, ctx: Context):
        ...
        if player.name not in self.persistent.disabled_players:
